...
- Book merging requires an exact subject section and tier. Cheese preparations contain two to seven distinct sourced components; partial preparations accept only missing components from the same dish. Preserve the 20-cell board, paid book and pantry stock, collection and recipe unlocks, return, swap and undo. Delivery is immediate; difficulty comes from budgeting and merging rather than timers.
...
- Preserve the single seven-letter answer and six guesses. New puzzles use the core pool four times in five and the challenge pool otherwise. Add target words only with a definition and a usage note. The broad allowed list and hard target pool serve different purposes.
