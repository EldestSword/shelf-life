...
`content.js` is the immutable editorial catalogue. `engine.js` exposes deterministic merge, request, reward, upgrade, daily-job and puzzle functions. Randomness uses the recovered seeded generator. Daily puzzle selection hashes the local calendar date with the fixed `ShelfLife.daily.v1.` prefix. Changing the target order changes daily answers, so preserve it for this release.

`art.js` draws integer-coordinate SVG pixel clusters. Lou’s full figure is 64 × 80 logical pixels, displayed at 1× or 2× depending on context. Four separate face drawings support idle, pleased, unimpressed and celebratory states. The room is a layered SVG with one addition per upgrade; characters are separate overlays so room resizing does not distort their pixels. Objects have distinctive recipe silhouettes. UI icons remain simple scalable outlines.

## Saves

The version-1 game state is wrapped as `{format:"shelf-life", revision, state}`. Local storage key: `shelf-life.save.v1`. The preceding valid write is kept at `.previous`. Exported files use the same envelope. Import shows a summary and requires the in-game confirmation before replacing state. Saves are limited to 250,000 UTF-8 bytes.
