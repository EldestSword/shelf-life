...
(function () {
  ...
  function perform(action, data = {}) {
    ...
    switch (action) {
      ...
      case 'need-info':
        if(data.kind==='book')itemInfo({kind:'book',tier:Number(data.tier),section:data.section});
        else {pantryIndex=C.recipes.findIndex(r=>r.id===data.id);recipeTab=0;recipeModal(pantryIndex,true);}
        break;
      case 'return-item': {
  ...
  window.SHELF_APP=Object.freeze({snapshot:()=>E.copy(state),page:()=>page,version:'1.2.1'});
