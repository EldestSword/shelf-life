...
function fresh(seed=839021,day=dateKey()){
 const s={version:VERSION,seed:seed>>>0,funds:60,xp:0,board:Array(SIZE).fill(null),upgrades:[],unlockedSections:['fiction','essays'],unlockedRecipes:['manchego','wensleydale'],theme:'teal',served:0,merges:0,dishes:0,puzzles:0,discovered:[],catalogued:[],claimed:[],visits:Array(C.visitors.length).fill(0),requests:[],orderIndex:0,started:false,settings:{sound:false,motion:true,contrast:false},daily:{date:day,served:0,claimed:false},dailyPuzzle:null,practicePuzzle:null,practiceCount:0,stats:{best:0},seenEnding:false};
...
function validatePuzzle(p){
 ...
 const common=p&&Array.isArray(p.targets)&&p.targets.every(x=>W.answers.includes(x))&&Array.isArray(p.guesses)&&p.guesses.every(x=>allowedWords.has(x))&&new Set(p.guesses).size===p.guesses.length&&typeof p.rewarded==='boolean'&&typeof p.date==='string'&&(p.date==='practice'||/^\d{4}-\d{2}-\d{2}$/.test(p.date))&&typeof p.input==='string'&&/^[A-Z]{0,7}$/.test(p.input);
 if(!common)throw Error('The word puzzle in this save is not valid.');
 // Version 1.1 saves may contain the retired two-board puzzle. Its rewards and
 // library progress remain intact; only the unfinished puzzle itself is reset.
 if(p.targets.length===2){if(p.targets[0]===p.targets[1]||p.guesses.length>9)throw Error('The word puzzle in this save is not valid.');return null;}
 if(p.targets.length!==1||p.guesses.length>6)throw Error('The word puzzle in this save is not valid.');
 const status=puzzleStatus(p);if(p.rewarded!==status.done||p.guesses.some((_,i)=>i<p.guesses.length-1&&p.guesses.slice(0,i+1).includes(p.targets[0])))throw Error('The puzzle completion record is inconsistent.');
 return {targets:p.targets.slice(),guesses:p.guesses.slice(),rewarded:p.rewarded,date:p.date,input:p.input};
}
function validate(x){
 ...
 const sectionIds=C.sections.map(v=>v.id),recipeIds=C.recipes.map(v=>v.id);
 const inferredSections=[...new Set(['fiction','essays',...s.board.filter(i=>i?.kind==='book').map(i=>i.section),...s.requests.map(r=>r.section),...s.catalogued.map(v=>v.split(':')[0])])];
 ...
 s.stats={best:Number.isInteger(x.stats?.best)&&x.stats.best>=0&&x.stats.best<=9?Math.min(x.stats.best,6):0};return s;
}
function rotateDay(s,day){if(s.daily.date!==day){s.daily={date:day,served:0,claimed:false};return true;}return false;}
function unlocked(s){return C.recipes.filter(r=>s.unlockedRecipes.includes(r.id));}
...
function bookPrice(sectionId){return sectionId?10:6;}
function buyBook(s,sectionId=null){const price=bookPrice(sectionId);if(sectionId&&!s.unlockedSections.includes(sectionId))return {ok:false,reason:'locked'};if(s.funds<price)return {ok:false,reason:'funds',price};if(!s.board.includes(null))return {ok:false,reason:'full'};const choices=s.unlockedSections.map(section).filter(Boolean),chosen=sectionId||choices[Math.floor(random(s)*choices.length)].id;const result=spawnBook(s,chosen);if(!result.ok)return result;s.funds-=price;return {...result,price};}
function buySection(s,id){const v=section(id);if(!v||s.unlockedSections.includes(id)||s.served<v.unlock||s.funds<v.price)return false;s.funds-=v.price;s.unlockedSections.push(id);return true;}
function buyRecipe(s,id){const r=recipe(id);if(!r||s.unlockedRecipes.includes(id)||s.served<r.unlock||s.funds<r.research)return false;s.funds-=r.research;s.unlockedRecipes.push(id);return true;}
function stockRecipe(s,id){const r=recipe(id);if(!r||!s.unlockedRecipes.includes(id))return {ok:false,reason:'locked'};if(s.funds<r.stock)return {ok:false,reason:'funds',price:r.stock};const empty=s.board.map((x,i)=>x===null?i:-1).filter(i=>i>=0);if(empty.length<r.components.length)return {ok:false,reason:'space',needed:r.components.length};const indices=empty.slice(0,r.components.length);r.components.forEach((c,n)=>{s.board[indices[n]]={kind:'ingredient',id,component:c.id};});s.funds-=r.stock;return {ok:true,indices,price:r.stock};}
...
function newRequest(s,slot){const rs=unlocked(s),serial=s.orderIndex++,visitor=Math.floor(random(s)*C.visitors.length),prefs=C.visitors[visitor].sections.filter(id=>s.unlockedSections.includes(id)),choices=prefs.length?prefs:s.unlockedSections;const chosen=choices[Math.floor(random(s)*choices.length)];const max=s.served<5?3:s.served<15?4:s.served<35?5:6;const tier=2+Math.floor(random(s)*Math.min(max-1,slot===1?2:max-1));const rid=slot===1?null:rs[Math.floor(random(s)*rs.length)].id;return {visitor,tier,section:chosen,recipe:rid,serial};}
function serve(s,index){const q=s.requests[index];if(!q)return {ok:false};const n=neededIndices(s,q);if(!n.ready)return {ok:false};const coins=reward(s,q);s.board[n.book]=null;if(q.recipe)s.board[n.dish]=null;s.funds+=coins;s.xp+=10+q.tier*2;s.served++;s.daily.served++;s.visits[q.visitor]++;s.requests[index]=newRequest(s,index);return {ok:true,coins,visitor:q.visitor};}
...
function makePuzzle(seed,answers,date='practice'){if(!answers.length)throw Error('Need at least one answer.');const rng={seed:seed>>>0};let pool=answers;if(answers===W.answers&&W.core?.length&&W.challenge?.length)pool=random(rng)<.8?W.core:W.challenge;const a=Math.floor(random(rng)*pool.length);return {targets:[pool[a]],guesses:[],rewarded:false,date,input:''};}
function puzzleStatus(p){const solved=[p.guesses.includes(p.targets[0])],won=solved[0];return {solved,won,done:won||p.guesses.length>=6};}
function submitGuess(s,p,word,allowed){word=word.toUpperCase();if(puzzleStatus(p).done)return {ok:false,message:'This puzzle is complete.'};if(!/^[A-Z]{7}$/.test(word))return {ok:false,message:'Seven letters, please.'};if(!allowed.has(word))return {ok:false,message:'Not in the offline word list. No guess used.'};if(p.guesses.includes(word))return {ok:false,message:'Already tried. No guess used.'};p.guesses.push(word);p.input='';const status=puzzleStatus(p);let coins=0;if(status.done&&!p.rewarded){p.rewarded=true;coins=status.won?80:0;s.funds+=coins;if(status.won){s.puzzles++;if(!s.stats.best||p.guesses.length<s.stats.best)s.stats.best=p.guesses.length;}}return {ok:true,...status,coins};}
const E={VERSION,SIZE,MAX_TIER,recipe,section,copy,hash,random,dateKey,fresh,validate,rotateDay,unlocked,compatible,move,spawnBook,bookPrice,buyBook,buySection,buyRecipe,stockRecipe,returnItem,neededIndices,reward,serve,buy,metric,claim,claimDaily,feedback,makePuzzle,puzzleStatus,submitGuess};
