...
function fresh(seed=839021,day=dateKey()){
 const s={version:VERSION,seed:seed>>>0,funds:60,xp:0,board:Array(SIZE).fill(null),upgrades:[],theme:'sage',served:0,merges:0,dishes:0,puzzles:0,discovered:[],claimed:[],visits:Array(C.visitors.length).fill(0),requests:[],orderIndex:0,started:false,settings:{sound:false,motion:true,contrast:false},daily:{date:day,served:0,claimed:false},dailyPuzzle:null,practicePuzzle:null,practiceCount:0,stats:{best:0},seenEnding:false};
 s.board[0]={kind:'book',tier:1};s.board[1]={kind:'book',tier:1};s.board[5]={kind:'cheese',id:'manchego'};s.board[6]={kind:'prep',id:'manchego'};s.board[10]={kind:'book',tier:1};s.board[11]={kind:'book',tier:2};s.requests=[{visitor:0,tier:2,recipe:'manchego',serial:0},{visitor:1,tier:2,recipe:null,serial:1},{visitor:2,tier:3,recipe:'wensleydale',serial:2}];s.orderIndex=3;return s;
}
...
function validate(x){
 ...
 if(!Array.isArray(x.requests)||x.requests.length!==3||!x.requests.every(r=>r&&Number.isInteger(r.visitor)&&r.visitor>=0&&r.visitor<C.visitors.length&&Number.isInteger(r.tier)&&r.tier>=1&&r.tier<=MAX_TIER&&(r.recipe===null||recipe(r.recipe))&&Number.isSafeInteger(r.serial)&&r.serial>=0))throw Error('The visitor requests are invalid.');
 s.requests=x.requests.map(r=>({visitor:r.visitor,tier:r.tier,recipe:r.recipe,serial:r.serial}));
 if(!Array.isArray(x.visits)||x.visits.length!==C.visitors.length||!x.visits.every(v=>Number.isSafeInteger(v)&&v>=0&&v<=100000000))throw Error('The visitor record is invalid.');s.visits=x.visits.slice();
 ...
 s.started=!!x.started;s.seenEnding=!!x.seenEnding;
 s.theme=['sage','rose','twilight'].includes(x.theme)?x.theme:'sage';
...
function compatible(a,b){if(!a||!b)return null;if(a.kind==='book'&&b.kind==='book'&&a.tier===b.tier&&a.tier<MAX_TIER)return {kind:'book',tier:a.tier+1};if(a.id===b.id&&recipe(a.id)&&((a.kind==='cheese'&&b.kind==='prep')||(b.kind==='cheese'&&a.kind==='prep')))return {kind:'dish',id:a.id};return null;}
function move(s,from,to,swap=false){if(!Number.isInteger(from)||!Number.isInteger(to)||from===to||from<0||to<0||from>=SIZE||to>=SIZE||!s.board[from])return {ok:false};const a=s.board[from],b=s.board[to],result=compatible(a,b);if(result){s.board[to]=result;s.board[from]=null;if(result.kind==='book'){s.merges++;s.xp+=2;}else{s.dishes++;s.xp+=4;if(!s.discovered.includes(result.id))s.discovered.push(result.id);}return {ok:true,type:result.kind==='book'?'merge':'dish',item:result};}if(!b||swap){s.board[to]=a;s.board[from]=b;return {ok:true,type:'move'};}return {ok:false,type:'different'};}
function spawnBook(s){const i=s.board.indexOf(null);if(i<0)return {ok:false,reason:'full'};let roll=random(s), tier=1;for(const [id,chance,level] of [['loft',.08,4],['rare',.15,3],['trolley',.25,2]]) { if(!s.upgrades.includes(id)) continue; if(roll<chance){tier=level;break;} roll-=chance; }s.board[i]={kind:'book',tier};return {ok:true,index:i,tier};}
function stockRecipe(s,id){const r=recipe(id);if(!r||r.unlock>s.served)return {ok:false,reason:'locked'};const empty=s.board.map((x,i)=>x===null?i:-1).filter(i=>i>=0);if(empty.length<2)return {ok:false,reason:'space'};s.board[empty[0]]={kind:'cheese',id};s.board[empty[1]]={kind:'prep',id};return {ok:true,indices:empty.slice(0,2)};}
function returnItem(s,i){if(!Number.isInteger(i)||i<0||i>=SIZE||!s.board[i])return false;s.board[i]=null;return true;}
function neededIndices(s,q){const a=s.board.findIndex(i=>i?.kind==='book'&&i.tier===q.tier);const b=q.recipe?s.board.findIndex(i=>i?.kind==='dish'&&i.id===q.recipe):-1;return {book:a,dish:b,ready:a>=0&&(!q.recipe||b>=0)};}
function reward(s,q){let book=10*Math.pow(2,q.tier-1),food=q.recipe?recipe(q.recipe).value:0;const has=id=>s.upgrades.includes(id);if(has('dresser'))food*=1.15;let mult=1+(has('nook')?.05:0)+(has('club')?.10:0)+(has('terrace')?.10:0)+(has('glass')?.10:0)+(has('festival')?.15:0);return Math.round((book+food)*mult)+(has('lamps')?10:0)+(has('atlas')&&q.recipe?20:0);}
function newRequest(s,slot){const rs=unlocked(s),serial=s.orderIndex++,visitor=Math.floor(random(s)*C.visitors.length),max=s.served<5?3:s.served<15?4:s.served<35?5:6;let tier=2+Math.floor(random(s)*Math.min(max-1,slot===1?2:max-1));const rid=slot===1?null:rs[Math.floor(random(s)*rs.length)].id;return {visitor,tier,recipe:rid,serial};}
...
function metric(s,key){if(key==='recipes')return s.discovered.length;if(key==='upgrades')return s.upgrades.length;return s[key]||0;}
...
const E={VERSION,SIZE,MAX_TIER,recipe,copy,hash,random,dateKey,fresh,validate,rotateDay,unlocked,compatible,move,spawnBook,stockRecipe,returnItem,neededIndices,reward,serve,buy,metric,claim,claimDaily,feedback,makePuzzle,puzzleStatus,submitGuess};
